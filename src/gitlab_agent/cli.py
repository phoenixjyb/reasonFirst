...
def _build_parser(prog: str = "gitlab-agent") -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--git-only",
        action="store_true",
        help="Operate without GITLAB_TOKEN; GitLab API/MCP/CI features remain unavailable",
    )
    ...
    p.add_argument(
        "--git-only",
        action="store_true",
        help="Use Git HTTPS only during preflight/start; API/MCP/CI features remain unavailable",
    )
    p.add_argument(
        "--no-launch",
        action="store_true",
