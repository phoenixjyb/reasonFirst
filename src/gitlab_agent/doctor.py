...
def run_doctor(
    *,
    ...
    try:
        ...
        return _finish(checks, offline=offline, git_only=git_only)
    ...
    api_status = "pass" if settings.api_token else ("skip" if git_only else "fail")
    _check(
        checks,
        "gitlab_api_token",
        api_status,
        (
            ...
            if settings.api_token
                ...
                "GitLab REST API, MCP metadata, and CI API features are unavailable"
                if git_only
    ...
    _check(
        ...
        credential_source=settings.git_credential_source,
        separate_git_token=settings.git_credential_source == "git_token",
        explicit_git_password=settings.git_credential_source == "git_password",
        scope_checked=False,
        value_exposed=False,
    )
    ...
    if git_only:
        _check(
            ...
            "skip",
            "GitLab API connectivity is disabled by explicit Git-only mode",
        )
    elif offline:
        _check(
            checks,
...
def _finish(
    ...
    *,
    ...
    return {
        ...
        "offline": offline,
        "git_only": git_only,
        "operating_mode": "git-only" if git_only else "api+git",
        "summary": counts,
        "checks": checks,
    }
